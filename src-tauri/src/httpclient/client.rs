...
use std::{
  collections::HashMap, path::PathBuf, time::Duration, sync::{ Arc, Mutex},
};
...
type ClientId = u32;
type ClientStore = Arc<Mutex<HashMap<ClientId, crate::httpclient::client::Client>>>;
...
impl ClientBuilder {
  ...
  /// Builds the Client.
  pub fn build(self) -> crate::err::Result<Client> {
...
impl Client {
  ...
  ///
  /// # Examples
  pub async fn send(&self, mut request: HttpRequestBuilder) -> crate::err::Result<Response> {
  ...
  //   request_builder = request_builder.headers(request_builder.credentials(Some(
  //     Credentials::new("same-origin")
  // )));
  //ai纯纯瞎写
      ...
      request_builder = match body {
        ...
        Body::Form(form_body) => {
          ...
          fn send_form(
            ...
            form_body: FormBody,
          ) -> crate::err::Result<reqwest::RequestBuilder> {
...
impl TryFrom<FilePart> for Vec<u8> {
  ...
  fn try_from(file: FilePart) -> crate::err::Result<Self> {
...
/// # Examples
/// ```rust,no_run
/// use crate::err::http::{HttpRequestBuilder, ResponseType, ClientBuilder};
/// async fn run() {
...
impl HttpRequestBuilder {
  ...
  pub fn new(method: impl Into<String>, url: impl AsRef<str>) -> crate::err::Result<Self> {
    Ok(Self {
      method: method.into(),
  ...
  /// Adds a header.
  pub fn header<K, V>(mut self, key: K, value: V) -> crate::err::Result<Self>
  where
...
impl Response {
  ...
  pub async fn bytes(self) -> crate::err::Result<RawResponse> {
  ...
  // let client = crate::err::http::ClientBuilder::new().build()?;
  // let mut stream = client.send(crate::err::http::HttpRequestBuilder::new("GET", "http://httpbin.org/ip")?)
  //   .await?
  //   .bytes_stream();
  //
  ...
  pub(crate) fn bytes_stream(
    self,
  ) -> impl futures_util::Stream<Item = crate::err::Result<bytes::Bytes>> {
    use futures_util::StreamExt;
  ...
  pub async fn read(self) -> crate::err::Result<ResponseData> {
    ...
    for (name, value) in self.1.headers() {
      ...
      raw_headers.insert(
        ...
        self
          ...
          .collect::<crate::err::Result<Vec<String>>>()?,
      );
    }
